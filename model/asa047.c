...
*/
{
  ...
  double ystar;
  double z;
  double rscale = 10.0; // 100.0, 1000.0
  long int zr, yrnewlo;
/*
...
*/
    ...
    *ifault = 0;
    ...
    for ( i = 0; i < n; i++ )
    {
      ...
      xmin[i] = xmin[i] + del;
      z = fn ( xmin, md );
      zr = (long int) rscale*z;
      // printf("\nlocal z = %ld, %ld", zr, yrnewlo);
      *icount = *icount + 1;
      if ( zr < yrnewlo )
      {
        *ifault = 2;
...
*/
  for ( ; ; )
  {
    ...
    y[n] = fn ( start, z_model, z_soundings, sounding_weights, n_soundings, spval);
    *icount = *icount + 1;
    ...
    for ( j = 0; j < n; j++ )
    {
      ...
      y[j] = fn ( start, z_model, z_soundings, sounding_weights, n_soundings, spval);
      *icount = *icount + 1;
      start[j] = x;
    }
/*                 
...
*/
      ...
      ystar = fn ( pstar, z_model, z_soundings, sounding_weights, n_soundings, spval);
      *icount = *icount + 1;
/*
  Successful reflection, so extension.
*/
      if ( ystar < ylo )
      {
        ...
        y2star = fn ( p2star, z_model, z_soundings, sounding_weights, n_soundings, spval);
        *icount = *icount + 1;
/*
  Check extension.
*/
...
*/
          if ( y[ihi] < y2star )
          {
            for ( j = 0; j < nn; j++ )
            {
              ...
              y[j] = fn ( xmin, z_model, z_soundings, sounding_weights, n_soundings, spval);
              *icount = *icount + 1;
            }
            ylo = y[0];
            ilo = 0;
...
*/
    ...
    *ifault = 0;
    ...
    for ( i = 0; i < n; i++ )
    {
      del = step[i] * eps;
      xmin[i] = xmin[i] + del;
      z = fn ( xmin, z_model, z_soundings, sounding_weights, n_soundings, spval);
      zr = (long int) rscale*z;
      // printf("\nlocal z = %ld, %ld", zr, yrnewlo);
      *icount = *icount + 1;
      if ( zr < yrnewlo )
...
*/
  for ( ; ; )
  {
    ...
    y[n] = fn ( start, z_model, z_soundings, sounding_weights, n_grids, n_soundings, spval);
    *icount = *icount + 1;
    ...
    for ( j = 0; j < n; j++ )
    {
      ...
      y[j] = fn ( start, z_model, z_soundings, sounding_weights, n_grids, n_soundings, spval);
      *icount = *icount + 1;
      start[j] = x;
    }
/*                 
...
*/
      ...
      ystar = fn ( pstar, z_model, z_soundings, sounding_weights, n_grids, n_soundings, spval);
      *icount = *icount + 1;
/*
  Successful reflection, so extension.
*/
      if ( ystar < ylo )
      {
        ...
        y2star = fn ( p2star, z_model, z_soundings, sounding_weights, n_grids, n_soundings, spval);
        *icount = *icount + 1;
/*
  Check extension.
*/
...
*/
          if ( y[ihi] < y2star )
          {
            for ( j = 0; j < nn; j++ )
            {
              ...
              y[j] = fn ( xmin, z_model, z_soundings, sounding_weights, n_grids, n_soundings, spval);
              *icount = *icount + 1;
            }
            ylo = y[0];
            ilo = 0;
...
*/
    ...
    *ifault = 0;
    ...
    for ( i = 0; i < n; i++ )
    {
      del = step[i] * eps;
      xmin[i] = xmin[i] + del;
      z = fn ( xmin, z_model, z_soundings, sounding_weights, n_grids, n_soundings, spval);
      zr = (long int) rscale*z;
      // printf("\nlocal z = %ld, %ld", zr, yrnewlo);
      *icount = *icount + 1;
      if ( zr < yrnewlo )
